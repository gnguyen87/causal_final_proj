...
leaflet() %>%
  ...
  addPolygons(
    data = redlining,
    fillColor = ~colorFactor(palette = "viridis", domain = as.factor(EQINTER20))(as.factor(EQINTER20)),
    fillOpacity = 0.7,
    color = "black", # Polygon border color
    weight = 1, # Border thickness
    label = ~paste("HOLC Grade:", as.factor(EQINTER20))
  ) %>%
  addCircleMarkers(
    data = racial_cov,
    radius = 0.1,
  ...
  addLegend(
    ...
    pal = colorFactor(palette = "viridis", domain = as.factor(redlining$EQINTER20)),
    values = ~as.factor(EQINTER20),
    title = "HOLC Grade"
  ) %>%
  addLegend(
    position = "bottomright",
